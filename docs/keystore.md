...
- `cyber` a.k.a. address. Is used for linking content, sending liquid tokens, delegating, redelegating, and undelegating tokens to validators. Also, withdrawing rewards and voting.
  - Derived from account keys generated by `cyberdcli keys add`
  - e.g. `cyber15h6vd5f0wqps26zjlwrc6chah08ryu4hzzdwhc`

- `cybervaloper` validator address. Is possible to delegate, redelegate and undelegate liquid tokens to/from this address abd invoke staking commands. Used to associate a validator to its operator
...
- `cybervalconspub` the public key of validator address.
