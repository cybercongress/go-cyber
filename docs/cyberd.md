...
A consensus computer enables computing of provably relevant answers without opinionated blackbox intermediaries, for instance: Google, Youtube, Amazon, or Facebook.
Stateless content-addressable peer-to-peer communication networks, like IPFS, and stateful consensus computers, like Ethereum, prove to be but a part of the solution, while there are at least three problems associated with their implementation.
Without a doubt, the first problem here is the subjective nature of relevance. The second problem is that scaling consensus computer for a huge knowledge graph is quite a sophisticated challenge. The third problem is that the quality of a knowledge graph of the kind will unavoidably suffer from different attack surfaces, such as sybil, selfish behavior of interacting agents.
In this paper, we: (1) define a protocol for provable consensus computing of relevance between IPFS objects (or any objects in any content-addressable p2p network) based on Tendermint consensus and cyber•rank computed on GPU, (2) discuss implementation details (3) and design distribution and incentive scheme based on our experience.
We believe the minimalistic architecture of the protocol is critical for the formation of a network of domain-specific knowledge consensus computers. As a result of our work, some applications that have never existed before emerge. We expand the work with our "after Genesis vision" on features and apps.

## Introduction to web3

The original Internet protocols, such as TCP/IP, DNS, URL, and HTTPS, molded the web into what it is now. Aside from all the advantages the protocols granted, they also brought more problems to the table. Being a vital property of the Internet since the moment of its inception, globality is under real threat. The speed of connections degrades with continuous network growth and due to governments ubiquitous interventions into web users' privacy and security. One property, which was not evident in the beginning, has grown immensely important with everyday usage of the Internet – its capability of exchanging permanent hyperlinks. [Thus they would not break with time](https://ipfs.io/ipfs/QmNhaUrhM7KcWzFYdBeyskoNyihrpHvUEBQnaddwPZigcN). Reliance on "one at a time ISP" architecture enables governments to censor packets effectively. It is the last straw in a conventional web stack for every engineer who is concerned about the future of our children.

Other properties, although not so critical, are very desirable: offline and real-time. When offline, a regular Internet user must be able to work with the state they have. However, with a connection established, they must be able to sync with a global state and continue to verify the state's validity in real-time while being connected.
Now, these properties offered on the app level while they must be integrated into lower-level protocols.

The emergence of a [web3 stack](https://ipfs.io/ipfs/Qmf3eHU9idMUZgx6MKhCsFPWL24X9pDUi2ECqyH8UtBAMQ) opens a large window of opportunity for a new kind of the Internet. Community calls it "web3". We call it "The Great Web" as we expect that some low-level conventions must become immutable and remain unchanged for decades (e.g., immutable content links). It has a promise to remove problems of a conventional protocol stack, better the speed, and add a more accessible web connection. However, as usual in a story with a new stack, new traps and pitfalls emerge. One of them is a general-purpose search. Existing general-purpose search engines are restrictive centralized databases everybody forced to trust. These search engines were designed primarily for client-server architecture based on TCP/IP, DNS, URL, and HTTPS protocols. Web3 brings up a challenge and an opportunity for a search engine based on developing technologies and designed specifically for them. Surprisingly, the permissionless blockchain architecture itself allows for organizing general-purpose search engines in a way inaccessible for previous architectures.

## On the problem of adversarial examples
...
Obviously, the cryptoeconomic approach can change beneficiaries in this game. Thus it effectively removes possible sybil attack vectors and eradicates the necessity to make a decision. For example, centralized ranking of web pages listed in a search results on the basis of centralized analysis of these web pages content and user behavior. Learning sybil-resilient model will probably lead to more predictive results.
