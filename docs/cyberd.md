...
[cyber•Congress](https://cybercongress.ai/): @xhipster, @litvintech, @hleb-albau
...
Merfle proof of [rank computation](https://github.com/cybercongress/cyberd/blob/master/docs/rank.md) by @arturalbov ...
...
After some research we found an algorithm that is probably satisfy all criteria: SpringRank. Original idea of an algorithm came from physics. Links represented as system of springs with some energy.
...
The problem with spring rank that we were not able to implement it fast using Go in `euler`. So we decide to find some more basic bulletproof way to boostrap the network: token weigted [PageRank](http://ilpubs.stanford.edu:8090/422/1/1999-66.pdf).
