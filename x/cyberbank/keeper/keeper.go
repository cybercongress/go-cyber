...
type IndexedKeeper struct {
	*Proxy
	...
	userTotalStakeAmper 	  map[uint64]uint64
	userNewTotalStakeAmper    map[uint64]uint64
...
func (k *IndexedKeeper) LoadState(rankCtx sdk.Context, freshCtx sdk.Context) {
	k.userTotalStakeAmper = make(map[uint64]uint64)
	k.accountKeeper.IterateAccounts(rankCtx, k.getCollectFunc(rankCtx, k.userTotalStakeAmper))

	k.userNewTotalStakeAmper = make(map[uint64]uint64)
	k.accountKeeper.IterateAccounts(freshCtx, k.getCollectFunc(freshCtx, k.userNewTotalStakeAmper))
}
...
func  (k *IndexedKeeper) UpdateStakeAmper(account uint64, stake uint64) {
	k.userNewTotalStakeAmper[account] = stake
}

func (k *IndexedKeeper) GetTotalStakesAmper() map[uint64]uint64 {
	return k.userTotalStakeAmper
}

func (k *IndexedKeeper) DetectUsersStakeAmperChange(ctx sdk.Context) bool {
	stakeChanged := false
	for o, n := range k.userNewTotalStakeAmper {
		if k.userTotalStakeAmper[o] != n {
			stakeChanged = true
			k.userTotalStakeAmper[o] = n
		}
	}
	return stakeChanged
}
...
func (k *IndexedKeeper) EndBlocker(ctx sdk.Context) {
	for _, addr := range k.accountToUpdate {
		...
		k.userNewTotalStakeAmper[accountNumber] = uint64(stake)
	}
